...
class TestAdminUploadEndpoint:
    ...
    def test_upload_file_success(self, mocker: MockerFixture):
        ...
        try:
            response = client.post("/admin/upload/", files={"file": ("test.txt", b"content", "text/plain")})
            assert response.status_code == 200
            assert response.json()["messege"] == "upload success"
    ...
    def test_upload_file_no_filename(self):
        app.dependency_overrides[get_current_admin] = lambda: {"user_id": 1, "is_admin": True}
        try:
            response = client.post("/admin/upload/", files={"file": ("", b"content", "text/plain")})
    ...
    def test_upload_file_invalid_extension(self):
        app.dependency_overrides[get_current_admin] = lambda: {"user_id": 1, "is_admin": True}
        try:
            response = client.post("/admin/upload/", files={"file": ("test.exe", b"content", "application/x-msdownload")})
...
class TestAdminIngestEndpoint:
    ...
    def test_ingest_document_success(self, mocker: MockerFixture):
        ...
        try:
            response = client.post("/admin/api/documents/1/ingest")
            assert response.status_code == 200
            assert response.json()["messege"] == "Ingestion started"
            assert response.json()["doc_id"] == 1
        finally:
            app.dependency_overrides.clear()
    ...
    def test_ingest_document_not_found(self, mocker: MockerFixture):
        ...
        mock_cursor.fetchone.return_value = None
        try:
            response = client.post("/admin/api/documents/999/ingest")
